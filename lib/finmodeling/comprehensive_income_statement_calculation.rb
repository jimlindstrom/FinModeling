  ...
  class ComprehensiveIncomeStatementCalculation < CompanyFilingCalculation
    ...
    def reformulated(period)
      ...
      return ReformulatedIncomeStatement.new(period,
                                             comprehensive_income_calculation.summary(:period=>period))
    end

    def latest_quarterly_reformulated(prev_stmt)
      ...
      if cur_period && prev_period
        new_re_is = reformulated(cur_period) - prev_stmt.reformulated(prev_period)
